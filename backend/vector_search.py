...
def build_index(max_features: int = 12000) -> dict[str, Any]:
    ...
    engine = "sklearn"
    if faiss is not None and matrix.shape[0]:
    ...
    return {"papers": len(papers), "dimension": int(matrix.shape[1]) if matrix.shape[0] else 0, "engine": engine}
