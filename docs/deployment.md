...
5. 部署成功后，Render 会生成形如 `https://nosql-scholar-system.onrender.com` 的公网地址。

注意事项：

- `data/processed/papers.json` 和 `data/neo4j/*.csv` 需要随仓库提交。
- `data/faiss/*.npy`、`data/faiss/*.index`、`data/faiss/*.pkl` 不需要提交，Render 会在部署时重新生成。
- Render 免费服务长时间无人访问可能会休眠，首次访问会有几十秒冷启动时间。
- 如果课程验收需要稳定访问，建议答辩前提前打开一次公网地址，等待服务唤醒。

## 3. MongoDB 与 Neo4j 说明

当前 Web 展示系统主要读取本地预处理后的 JSON/CSV/向量索引文件，因此 Render 公网部署不强制依赖在线 MongoDB 或在线 Neo4j 服务。

MongoDB 和 Neo4j 主要用于课程报告中的数据库设计、导入脚本和查询分析说明：
